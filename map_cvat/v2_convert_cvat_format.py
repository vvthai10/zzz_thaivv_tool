...
# TODO: Read folder "run CVAT format"
json_format_folder_path = os.path.join(dpath, "run CVAT format")
json_file_names = glob.glob(json_format_folder_path + "/*.json", recursive = True)
for json_file in json_file_names:
    ...
    # TODO Create cvat zip
    new_folder = Path(json_file).parts[-1].split(".")[0].replace("Copy of ", "")
    cvat_path = os.path.join(dpath, "cvat_format", new_folder)
    if os.path.exists(cvat_path):
        shutil.rmtree(cvat_path) 
    ...
    ZIP_NAME = os.path.join(dpath, "cvat_format", new_folder)
